...
const title = "Počty případů 2024";
...
const chartOptions: ApexOptions = {
  ...
  colors: [getCSSVariableValue("--bs-warning"), getCSSVariableValue("--bs-info")],
};
  ...
  (newValue) => {
    if (newValue) {
      series.value = [
        {
          name: "Zakázky",
          data: newValue.months.map((item) => item.orders.all.count),
        },
        {
          name: "Nabídky",
          data: newValue.months.map((item) => item.offers.all.count),
        },
      ];
    }
  }
